...
Recorded: 2026-09-18 · **Revised: 2026-09-20** · Authority: Meta Prompt §D3, Master §41 · Environment: this development host, direct outbound HTTPS
...
Live checks are opt-in: `ONESHELF_LIVE_SOURCES=1 .venv/bin/pytest tests/live -m live` (7 passed, 2026-09-20).
...
## 1b. The three sources completed on 2026-09-20, capability by capability
...
| Capability | WEBTOON | Tapas | Safahat (Hindawi) |
|---|---|---|---|
| Search | **Unsupported** — robots.txt disallows `/*/search` for every agent | **Unsupported** — robots.txt disallows `/search` for every agent | **Confirmed** — five results with titles, ids and authors |
| Work Details | **Confirmed** — title and cover | **Confirmed** — title, cover and synopsis from `/info` | **Confirmed** — title, author, cover and synopsis |
| Catalog / Reading Units | **Confirmed** — >300 episodes, complete, in the source's own order and numbering | **Confirmed** — 721 episodes paginated to completion | **Confirmed** — one book, one Reading Unit (`one_shot`) |
| Reader resources | **Confirmed** — images extracted and fetched | **Confirmed** — images extracted and fetched | **Unsupported** — a book is a file, read by OneShelf's own document reader |
| Download resources | **Confirmed** — three images validated by the download path's own checks and packaged into a CBZ the integrity validator accepts | **Confirmed** — an episode image passed the same validator | **Confirmed** — EPUB and PDF offered; a 2.3 MB EPUB opened as an EPUB |
| Latest | **Unknown** — not investigated, and not required by §41 | **Unknown** — not investigated | **Unknown** — not investigated |
| Auth / session | Not applicable — nothing here is behind a login, and nothing attempts one | Not applicable — episodes a series does not offer anonymously are simply not listed | Not applicable — the library is free to read |
...
|---|---|---|
| 3asq / Al-Aasheq | **unavailable** | `3asq.org` does not resolve from this environment — re-checked 2026-09-20, same result as 2026-09-18 (`getent hosts 3asq.org` returns nothing). No capability has been exercised and nothing about it is claimed; no substitute verification was invented for it. Per §41.4 it is a public catalogue/parser stress source, never an assumed official or licensed default. Unblocked only by network access to it. |
