...
| I-47 | **Found on the real deployed UI:** a live search result without a Work rendered as an inert span — every result of a real search could be seen but not opened (`WorkCard` only linked a `work_id`; `/api/listings/bind` alone would have left a new Work with no Reading Units). Home's Trending/Latest had the same hole. Previous UI suites were green because their fixtures always had a `work_id` | `POST /api/listings/open` binds exactly the chosen listing and brings details and catalogue; `ResultCard` makes every result a link or button, with a source/language chooser for grouped results; verified in a real browser to the Reader | Fixed |
| I-48 | **Found on the real deployed UI:** no cover reached any screen — `LiveListing.cover_url` was dropped by grouping, never stored, and no Work/Shelf/Home response carried one; Work Details rendered an empty box | Migration 0015 `source_listings.cover_url` (presentation only, INV-28); covers chosen per selected track; served same-origin through `/api/covers` under the source's network policy; verified in a real browser, English and Arabic | Fixed |
| I-49 | Found while verifying I-47: `RecipeRuntime.run`'s single-document path let its private `_PageFailure` escape instead of `CapabilityError`, so a real Gutenberg timeout made opening answer 500 (URL preview had the same hole) | Raised as `CapabilityError`; opening reports source failures and still returns the Work | Fixed |
| I-50 | Work Details' empty state told people to refresh the catalogue from the Sources tab, but no control did | Refresh catalogue on the selected source track | Fixed |
| I-51 | MangaDex `search` reports a series' *original* language while its catalogue is keyed by *translated* language, so opening a MangaDex result makes e.g. a Japanese track with no chapters. Substituting English would be the silent language switch M3.1 forbids | Adapter-contract change needed in OneShelf-Adapters (model available translations per language), then a version bump and re-publish. The Work still opens and can be refreshed | Open |
| I-52 | Unknown language (`und`) is displayed as "root" by the browser's language-name API in search cards | Cosmetic; display a translated "Unknown language" instead | Open |
| I-53 | While a search is still streaming, Retry chips appear for sources that have not answered yet | Offer Retry only for sources that reported a failure | Open |
