...
| M26.15 | Continue Reading restores exact same-source progress; Long Strip approximate | C2 | be/reader, fe/home | Home, Work Details | progress | `opens a unit where it was left rather than at the beginning`, `brings the stored page into view in Long Strip`, `clamps a stored position that is past the end of the unit`, `starts at the beginning when the library holds no position`, `writes nothing merely by resuming`, `opens the book at the chapter it was left on`, `opens the document at the page it is given`, `is given the page the library holds, through the Book Reader that renders it`; live locator round-trip against the running API (WP-R1) | VERIFIED |
