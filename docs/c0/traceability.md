...
## Release requirements (explicit user requirement, 2026-09-21)
...
| REL-06 | Release hygiene: clean tree, `main`, reviewed `.gitignore`, no secrets or user data tracked | `.gitignore` | A content scan of all 533 tracked files for private keys, cloud keys, tokens, bearer headers, cookies and password assignments found nothing; `.env`, `deploy/volumes/`, `*.osbackup` and local databases are ignored; the only credential present is the Test Source's deliberate fake, which `test_browser.py` asserts never reaches disk | VERIFIED |
| REL-07 | A public GitHub repository named exactly `OneShelf`, `main` pushed, remote HEAD equal to local HEAD | — | **Blocked on REL-08**: §11 of the requirement makes publishing conditional on the container gate passing | BLOCKED |
...
| REL-09 | Clean-clone release test against the published repository | — | **Blocked on REL-07 and REL-08** | BLOCKED |
