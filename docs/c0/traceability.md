...
| M41.1 | Real sources: MangaDex, 3asq/Al-Aasheq, WEBTOON, Tapas, Safahat/Hindawi (A5), Project Gutenberg, arXiv, Standard Ebooks | C3→C9 | plugins/official | Sources | plugins | **Seven of eight ship**, each passing the tests it packages (`test_official_packages.py`) and verified live on 2026-09-20 (`tests/live/test_source_suite.py`, `ONESHELF_LIVE_SOURCES=1`): MangaDex, Gutenberg, arXiv, Standard Ebooks, **WEBTOON** (work, catalog from its own episode API, reader from the viewer's own markup), **Tapas** (work, catalog from the site's own XHR, reader from the episode's own markup), and **Safahat/Hindawi** (search, work, catalog, downloads — EPUB and PDF, verified by opening a real 2.3 MB EPUB). None needed a browser: each was reachable statically or through the site's own request, which is what the earlier "renders with JavaScript" note got wrong. Search is `Unsupported` for WEBTOON and Tapas because both robots.txt files disallow it. **Still blocked:** 3asq/Al-Aasheq — `3asq.org` does not resolve from this environment (re-checked 2026-09-20); unblocked only by network access to it | IN_PROGRESS |
