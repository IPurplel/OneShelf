...
| D-C3-16 | Registry sources: `file://` local mirror or `https://` static index (client allowlisted to that host only). Configured by `ONESHELF_REGISTRY_URL`; no default registry. | A1 | Adopted; **default superseded 2026-09-21** by explicit user decision: Compose and `.env.example` default to this repository's Official Source Registry over HTTPS (REL-12) |
