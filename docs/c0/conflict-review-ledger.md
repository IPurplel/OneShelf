...
| A5 | "Safahat / Hindawi" canonical domain/identity isn't stated | §41.1, §41.3 | **Settled 2026-09-20.** `www.hindawi.org` now redirects to `www.safahat.org`, which is therefore the canonical domain; the books themselves are served from `downloads.hindawi.org`, so both names appear in the adapter — one as the source domain, one as its CDN. The shipped `oneshelf.hindawi` package records this, and a live check followed the redirect and opened a real EPUB. |
...
| D-C3-16 | Registry sources: `file://` local mirror or `https://` static index (client allowlisted to that host only). Configured by `ONESHELF_REGISTRY_URL`; no default registry. | A1 | Adopted; **default superseded 2026-09-21** by explicit user decision: Compose and `.env.example` default to this repository's Official Source Registry over HTTPS (REL-12); **moved 2026-09-21** to the OneShelf-Adapters Registry (REL-21) |
