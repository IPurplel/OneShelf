...
on first run from a release snapshot included in OneShelf — no network needed — and marked
**Official · Bundled** on the Sources screen, where you can see exactly what each
...
The Sources screen also shows the **Source Registry**: adapters published from
[OneShelf-Adapters](https://github.com/IPurplel/OneShelf-Adapters), where adapters are developed and contributed. Each entry carries its own trust
level — Official, Verified Community or Community — and installs only after the same review and packaged tests.
